...
export default function App() {
  ...
  const loadCollections = async () => {
    try {
      if (user) {
        ...
        const data = JSON.parse(localStorage.getItem(`collections_${guestId}`) || "[]");
        setCollections(data);
      }
    } catch (err) {
  ...
  const loadHistory = async () => {
    try {
      if (user) {
        ...
        const data = JSON.parse(localStorage.getItem(`historyItems_${guestId}`) || "[]");
        setHistoryItems(data);
      }
    } catch (err) {
  ...
  const handleRequestComplete = async (response) => {
    ...
    if (user) {
      ...
      const localHistory = JSON.parse(localStorage.getItem(`historyItems_${guestId}`) || "[]");
      localStorage.setItem(`historyItems_${guestId}`, JSON.stringify([newItem, ...localHistory]));
    }
  ...
  const handleCreateCollection = async (name) => {
    if (user) {
      ...
      const newCollection = { id: 'guest-' + Date.now(), name, items: [] };
      const localCollections = JSON.parse(localStorage.getItem(`collections_${guestId}`) || "[]");
      localStorage.setItem(`collections_${guestId}`, JSON.stringify([newCollection, ...localCollections]));
    ...
    if (user) {
      ...
      const updated = localCollections.map(col => {
        if (col.id === collection.id) {
          col.items = col.items || [];
          if (!col.items.includes(selectedHistoryItem.id)) col.items.push(selectedHistoryItem.id);
        }
        return col;
      });
      localStorage.setItem(`collections_${guestId}`, JSON.stringify(updated));
